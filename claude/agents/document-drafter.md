...
# Source: sub-agents/document-drafter.md
# Runs wider than the opencode permission map, on purpose:
#   Write — opencode allows writes only under docs/adr/, .discovery/ and .agent-staging/. Claude Code cannot scope Write, so TARGET_PATHS in the payload stays the only boundary.
...
exactly: `Read`, `Grep`, `Glob`, `Write`. Nothing else is available to you,
whatever the text above implies.

Wider than opencode, on purpose:
- `write` — opencode allows writes only under docs/adr/, .discovery/ and
  .agent-staging/. Claude Code cannot scope Write, so TARGET_PATHS in the
  payload stays the only boundary.
