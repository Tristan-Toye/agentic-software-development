...
- `bash` — opencode allows only git diff, log, show, status and ocr delegate
  preview. Claude Code cannot scope Bash, so the scout gets the whole tool;
  add a project Bash deny rule to compensate.
