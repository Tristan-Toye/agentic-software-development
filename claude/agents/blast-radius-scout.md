...
# Source: sub-agents/blast-radius-scout.md
# Runs wider than the opencode permission map, on purpose:
#   Bash — opencode allows only git diff, log, show and status. Claude Code cannot scope Bash, so the scout gets the whole tool; add a project Bash deny rule to compensate.
...
exactly: `Read`, `Grep`, `Glob`, `Bash`. Nothing else is available to you,
whatever the text above implies.

Wider than opencode, on purpose:
- `bash` — opencode allows only git diff, log, show and status. Claude Code
  cannot scope Bash, so the scout gets the whole tool; add a project Bash
  deny rule to compensate.
