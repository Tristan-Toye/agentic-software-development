...
BANNED_TERMS = {
    "utilise": "use", "utilize": "use", "leverage": "use", "employ": "use",
    "begin": "start", "commence": "start", "initiate": "start",
    "cease": "stop", "halt": "stop", "terminate": "stop",
    "create": "make", "generate": "make", "produce": "make", "construct": "make",
    "modify": "change", "alter": "change", "adjust": "change", "tweak": "change",
    "revise": "change",
    "delete": "remove", "eliminate": "remove", "purge": "remove",
    "verify": "check", "validate": "check", "confirm": "check", "ensure": "check",
    "locate": "find", "discover": "find", "identify": "find",
    "resolve": "fix", "remediate": "fix",
    "issue": "problem", "defect": "problem", "flaw": "problem",
...
PRONOUN_RE = re.compile(r"^(This|That|These|Those|It|Which)\s+(is|are|was|were|"
                        r"means|makes|gives|has|have|will|can|does|do|should)\b")
# R4 — passive voice heuristic
PASSIVE_RE = re.compile(r"\b(is|are|was|were|be|been|being)\s+(\w+ed|done|made|"
                        r"given|taken|written|built|known|held|kept|sent|read)\b", re.I)
# R7 — noun cluster heuristic: 4+ consecutive lowercase words with no function word
FUNCTION_WORDS = {
    "the", "a", "an", "of", "in", "on", "to", "for", "with", "by", "from", "at",
    "and", "or", "but", "if", "as", "that", "than", "then", "so", "is", "are",
    "was", "were", "be", "not", "no", "it", "its", "this", "these", "each",
    "every", "any", "all", "one", "two", "three", "when", "where", "how", "why",
    "what", "who", "into", "over", "under", "after", "before", "during", "per",
    "do", "does", "did", "can", "may", "must", "will", "would", "should", "has",
    "have", "had", "run", "runs", "ran",
    "through", "across", "within", "without", "between", "against", "about",
    "above", "below", "beyond", "upon", "since", "until", "while", "because",
    "therefore", "such", "same", "other", "both", "only", "also", "still",
    "new", "old", "own", "more", "most", "less", "least", "many", "much",
}
...
VAGUE_CRITERION_WORDS = {
    "efficient", "efficiently", "fast", "quick", "quickly", "correct",
    "correctly", "properly", "robust", "clean", "good", "better", "reasonable",
    "appropriate", "acceptable", "sane", "sensible", "performant", "scalable",
    "maintainable", "readable", "nice",
}
...
STUB_MARKERS = (
    "notimplemented", "todo!", "unimplemented!", "pass", "...", "abstract",
    "throw new notimplementedexception", "raise notimplementederror",
)
...
def heading_order(body: str) -> list[str]:
    return [ln.strip() for ln, fenced in _fenced_lines(body)
...
def check_ste(where: str, text: str, rep: Report) -> None:
    ...
    for _, line in lines:
        for sentence in split_sentences(line):
            ...
            if n > 25:
                rep.warn(where, f"R2 sentence has {n} words (max 25): "
                                f"{sentence[:70]}...")
            elif n > 20:
                rep.warn(where, f"R2 sentence has {n} words (20 is the "
                                f"instruction limit): {sentence[:70]}...")

            low = [w.lower() for w in words]
            for w in low:
                if w in BANNED_TERMS:
                    rep.defect(where, f"R1 '{w}' is banned; use "
            ...
            if PRONOUN_RE.match(sentence.strip()):
                rep.defect(where, f"R8 unanchored pronoun as subject: "
                                  f"{sentence[:70]}...")
            if PASSIVE_RE.search(bare):
                rep.warn(where, f"R4 possible passive voice: {sentence[:70]}...")
            if " and " in bare.lower() and n > 18:
                rep.warn(where, f"R3 long sentence joined by 'and'; consider a "
                                f"split: {sentence[:60]}...")
            ...
            for w in low:
                ...
                run += 1
                if run >= 4:
                    rep.warn(where, f"R7 noun cluster of 4 or more: "
                                    f"{sentence[:70]}...")
                    break
...
def check_headings(where: str, body: str, expected: list[str], rep: Report) -> None:
    ...
    if found != expected:
        ...
        if not missing and not extra:
            rep.defect(where, f"sections are out of order; expected "
                              f"{' -> '.join(expected)}")
...
def check_anchors(where: str, fm: dict, repo_root: Path, rep: Report) -> None:
    ...
    for a in anchors:
        ...
        if not target.exists():
            rep.defect(where, f"anchor '{a}' points at a file that does not "
                              f"exist")
            continue
        ...
        if int(line_part) > n:
            rep.defect(where, f"anchor '{a}' is past the end of the file "
                              f"({n} lines)")


def check_evidence(where: str, text: str, rep: Report) -> None:
    if not any(lbl in text for lbl in EVIDENCE_LABELS):
        rep.defect(where, "no evidence label found; every claim needs one of "
...
def parse_packages(text: str) -> list[tuple[str, list[str]]]:
    ...
    for line in text.split("\n"):
        ...
        if len(cells) < 2 or cells[0].lower() in ("package", ""):
            continue
        paths = [p.strip() for p in cells[1].split(",")
                 if p.strip() and p.strip() != "—"]
        rows.append((cells[0], paths))
    return rows


def check_disjoint(where: str, rows: list[tuple[str, list[str]]],
                   rep: Report) -> None:
    if not rows:
    ...
    for name, paths in rows:
        ...
        for p in paths:
            ...
            if norm in owner:
                rep.defect(where, f"path '{p}' is owned by both "
    ...
    for a_norm, a_name in owner.items():
        for b_norm, b_name in owner.items():
            ...
            if b_norm.startswith(a_norm.rstrip("/") + os.sep):
                rep.defect(where, f"'{b_norm}' ({b_name}) is inside "
...
def check_contract(where: str, text: str, rep: Report) -> None:
    ...
    if not re.search(r"(///|//!|\*|\"\"\"|'''|<summary>|/\*\*|#)", body_text):
        rep.defect(where, "## Contract has no documentation comment; every "
    ...
    for i, line in enumerate(body_text.split("\n"), 1):
        ...
        if BODY_RE.match(line) or ASSIGN_RE.match(line):
            rep.defect(where, f"## Contract line {i} looks like a body, not a "
...
def check_criteria(where: str, text: str, rep: Report) -> None:
    ...
    for item in items:
        low = item.lower()
        vague = sorted(w for w in VAGUE_CRITERION_WORDS
                       if re.search(rf"\b{w}\b", low))
        concrete = bool(re.search(r"\d", item) or "`" in item)
        if vague and not concrete:
            rep.defect(where, f"criterion is not falsifiable ({', '.join(vague)}"
                              f" with no observable value): {item[:70]}")
        elif not concrete:
            rep.warn(where, f"criterion names no concrete value or identifier: "
                            f"{item[:70]}")
...
def validate_dossier(path: Path, repo_root: Path) -> Report:
    ...
    if status not in DOSSIER_STATUSES:
        rep.defect(where, f"status '{status}' is not one of "
                          f"{sorted(DOSSIER_STATUSES)}")
    ...
    if "## Work packages" in sec:
        check_disjoint(f"{where} ## Work packages",
                       parse_packages(sec["## Work packages"]), rep)
    if "## Acceptance criteria" in sec:
        check_criteria(f"{where} ## Acceptance criteria",
                       sec["## Acceptance criteria"], rep)

    if status == "ready" and not fm.get("baseline_commit"):
        rep.defect(where, "status is 'ready' but baseline_commit is empty; "
                          "/plan stamps it last")
...
DECISION_VERBS = (
    "use", "reject", "keep", "remove", "move", "split", "merge", "replace",
    "add", "drop", "coalesce", "defer", "adopt", "stop", "start", "make",
    "change", "fix", "hold", "cache", "batch", "isolate", "expose", "hide",
)
...
def validate_adr(path: Path, repo_root: Path) -> Report:
    ...
    if fm.get("status") not in ADR_STATUSES:
        rep.defect(where, f"status '{fm.get('status')}' is not one of "
                          f"{sorted(ADR_STATUSES)}")
    if not re.fullmatch(r"ADR-\d{4}", str(fm.get("id", ""))):
        rep.defect(where, f"id '{fm.get('id')}' must look like ADR-0007")

    title = str(fm.get("title", ""))
    if not any(re.search(rf"\b{v}\w*\b", title, re.I) for v in DECISION_VERBS) \
            and ";" not in title:
        rep.warn(where, f"title may state only the subject: '{title}' — a title "
    ...
    if "## Decision" in sec:
        sents = split_sentences(" ".join(
            ln for _, ln in prose_lines(sec["## Decision"])))
        if not sents:
            rep.defect(f"{where} ## Decision", "section is empty")
        else:
            n = len(re.findall(r"[A-Za-z']+", strip_technical(sents[0])))
            if n > 25:
                rep.defect(f"{where} ## Decision",
                           f"the first sentence is the decision and has {n} "
                           f"words; state one decision in one sentence")
    if "## Alternatives" in sec and not re.search(r"^\s*[-*]\s+\S",
                                                  sec["## Alternatives"], re.M):
        rep.warn(f"{where} ## Alternatives",
...
def adr_rows(adr_dir: Path) -> list[tuple[str, str, str]]:
    ...
    for p in sorted(adr_dir.glob("*.md")):
        ...
        try:
            ...
            continue
        rows.append((str(fm.get("id", "?")), str(fm.get("status", "?")),
                     str(fm.get("title", "?"))))
    return sorted(rows)
...
def check_index(adr_dir: Path, rep: Report) -> None:
    ...
    if not index.exists():
        if rows:
            rep.defect("adr/index.md", "the index is missing; regenerate it "
                                       "with --write-index")
        return
    if index.read_text() != render_index(rows):
        rep.defect("adr/index.md", "the index does not match the ADR front "
...
def selftest() -> int:
    with tempfile.TemporaryDirectory() as tmp:
        ...
        bad = GOOD_DOSSIER.replace("| UT unit tests | tests/unit/test_flush.py",
        ...
        print("\n--- selftest: a fenced '## ' in the build log is not a "
              "section ---")
        ...
        return rc if caught and clean else 1
...
def main() -> int:
    ...
    if args.all or (not args.dossier and not args.adr):
        ...
        targets_a = [p for p in sorted(adr_dir.glob("*.md"))
                     if p.name != "index.md"] if adr_dir.exists() else []
